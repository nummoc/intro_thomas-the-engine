...
void KEYDBNC_Process(void) {
   ...
   */
	DBNC_KeySet keys = KEYDBNC_GetKeys();
	DBNC_KeyStateKinds state = KEYDBNC_FSMdata.state;
  if (state == DBNC_KEY_IDLE && keys !=0) { /* a key is pressed and we are not debouncing */
  #if PL_HAS_KBI
