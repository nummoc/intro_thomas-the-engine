...
static volatile uint16_t BOOST_Scale = 300;
...
static int16_t scaleJoystickTo1K(int8_t val) {
  ...
  if (val>0) {
    tmp = ((val*10)/127)*BOOST_Scale;
  } else {
    tmp = ((val*10)/128)*BOOST_Scale;
  }
  if (tmp < -(10 * BOOST_Scale)) {
    tmp = -(10 * BOOST_Scale);
  } else if (tmp> (10 * BOOST_Scale)) {
    tmp = (10 * BOOST_Scale);
  }
  return tmp;
}
...
uint8_t REMOTE_HandleRemoteRxMessage(RAPP_MSG_Type type, uint8_t size, uint8_t *data, RNWK_ShortAddrType srcAddr, bool *handled, RPHY_PacketDesc *packet) {
  ...
  switch(type) {
#if PL_HAS_MOTOR
	  case RAPP_MSG_TYPE_BOOST:
		  *handled = TRUE;
		  boost = (data[0])|(data[1]<<8);
		  BOOST_Scale = boost;
		  break;
	  case RAPP_MSG_TYPE_TOGGLE_MODE:
		  *handled = TRUE;
		  if (REMOTE_useJoystick) {
