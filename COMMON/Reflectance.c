 ...
 */
static int ReadLine(SensorTimeType calib[REF_NOF_SENSORS],
		SensorTimeType raw[REF_NOF_SENSORS], bool white_line) {
	int i;
...
static uint8_t PrintStatus(const CLS1_StdIOType *io) {
	...
	CLS1_SendStatusStr((unsigned char*) "  line val", (unsigned char*) "",
			io->stdOut);
	buf[0] = '\0';
	UTIL1_strcatNum16s(buf, sizeof(buf), refCenterLineVal);
...
int startStopCallibration = 0;

void REF_StartStopCallibration(void) {
	startStopCallibration = 1;
}

int isStartStopCallibration(void) {
	return startStopCallibration;
}
